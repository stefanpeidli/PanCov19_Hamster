...
#First, investigate doublets so that one can set a reasonable threshold
library(DoubletFinder)

data_dir <- '/Volumes/fast/AG_Landthaler/tmp/ML_EW_096/scRNAseq_3dpianimal_1_lung/scRNAseq_3dpianimal_1_lung/outs/raw_feature_bc_matrix/'
data <- Read10X(data.dir = data_dir)

seu = CreateSeuratObject(counts = data, project="pr_seu", min.cells=5, min.features=250)
...
#This is done in folder /fast/AG_Landthaler/tmp/ML_EW_096/ma_new_lung
...
ma_new@meta.data$hamster <- gsub("ma_([de0-9]*)_lung_([0-9])","Ha\\2",ma_new@meta.data$orig.ident)
...
#######################
#Run hamster_merging_lung_cellstokeep.R, which does the thresholding using cells_to_keep and then integrates

#read integrated object from cluster
ma.int <- readRDS("/Volumes/fast/AG_Landthaler/tmp/ML_EW_096/seu_lung_new_combined_integrated.rds")
...
unfilt_metadata <- read.table("/Volumes/fast/AG_Landthaler/tmp/ML_EW_096/ma_new_lung/seu_lung_new_combined_250_metadata.txt")
ma.int@meta.data$cell_id <- rownames(ma.int@meta.data)
df <- merge(ma.int@meta.data, unfilt_metadata, by="cell_id")
...
saveRDS(ma.int, "/Volumes/fast/AG_Landthaler/tmp/ML_EW_096/seu_lung_new_combined_integrated_annotated.rds")
...
ggplot()+
  ...
  theme(axis.ticks=element_blank(), panel.grid.minor=element_blank())+
  scale_colour_manual(values = celltypecolors)
ggsave("seu_lung_new_integrated_celltypes.pdf", useDingbats=FALSE, width=14, height=7)
...
tgc = tgc %>% mutate(celltype = forcats::fct_relevel(celltype, the_celltypes))
ggplot(tgc, aes(x=celltype, y=fraction, fill=paste(celltype, timepoint, sep="_")))+
  geom_bar(aes(colour=timepoint), position=position_dodge(), stat="identity", size=0.2)+
  geom_point(data=c, aes(x=celltype, y=fraction, fill=paste(celltype, timepoint, sep="_")), position=position_jitterdodge(jitter.width = 0.1, dodge.width = 0.8), size=0.2)+
  geom_errorbar(aes(ymin=fraction-sd, ymax=fraction+sd), width=.2, position=position_dodge(.8), size=0.25)+
  theme_bw()+
  theme(axis.ticks=element_blank(), panel.grid.minor=element_blank())+
...
ggsave("seu_lung_new_integrated_celltypepercentage_pertimepoint.pdf", useDingbats=FALSE)
